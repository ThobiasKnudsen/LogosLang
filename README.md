...
`.` reads the fields a value's type defines, and a value's type is not one of them. Its type is read with `:`, which reads a name's record rather than its value: `x:type` is the type of what `x` holds, `x:scope` the scope `x` was declared in, `x:name` its spelling.
