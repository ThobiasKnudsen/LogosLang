...
- A **one-pass, self-directing parser/elaborator**: Pratt precedence driver running native `constructor`s, on-demand lex/parse interleaving, scopes + name resolution.
...
- The **`fn` execution model**: `interpret` (tree-walk), `compile` (Cranelift → `bcode`), `run` (invoke `bcode`); stack calling convention; ~30–40 native endpoint operations.
...
seed/                           # single binary+lib crate
  ...
  src/
    ...
    lex/
      trie.rs          ⚠        # regex-trie: 256-array literal path + regex-crate leaves; incremental insert
      token.rs                  # persistent token record {denoted, start, len}
    parse.rs           ⚠        # Pratt driver; on-demand lex; runs native constructors
    resolve.rs                  # scope tree + name→handle (rides the trie); the lexical `gate` access check (fail-closed, erases)
    interp.rs                   # fn.interpret tree-walk + stack frames
...
**Phase 2 ⚠ — Lexer.** Port `regex_trie.zig` to `trie.rs` using the `regex` crate for leaves; literal declarations are pure-literal inserts, regex tokens recompile one node. Emit token records `{denoted, start, len}`. *Test:* lex `a = a + 1`, lex `«…»` strings and `#` comments, insert a new token then lex a use of it. *Mostly grind; the incremental-insert correctness is the careful bit.*

**Phase 3 ⚠ — One-pass parser/elaborator.** Pratt driver keyed on float `precedence` + `associativity`; pulls tokens on demand (so declarations land in the trie before later tokens lex); runs each token's native `constructor`; resolves names via `resolve.rs`. Constructors for `:`,`=`,`:=`, `struct`,`array`,`mut`,`fn`, application, `+`/operators (monomorphizing in the constructor — `rational_number` literals mold to the sibling operand's type here). *Test:* parse `a = a + 1` and the `type`/`struct` defs into the LG shape the sketch shows. *The heart; mixed grind + care.*

**Phase 4 — Interpret tier.** `fn.interpret` tree-walk; stack frames for locals (recursion works here); the ~30–40 endpoint natives (their `run`): binding, field/apply, arith/compare, `if`/`else`/`while`/`for`, `alloc`/`drop`/`&`/`@`, literal eval, `error`. *Mostly grind.*
...
1. **Name-index correctness.** Every structural edit maintains the trie *as part of the edit*. Needs: a back-edge on each *declaration* dyad to its trie entry (declaration-only metadata — anonymous nodes carry nothing); the trie leaf keyed by owning code-scope → dyad, so removal is *drop the dying scope's key* (siblings/outer stay; nearest-ancestor resolution stays correct; no restore step). A stale global index would silently mis-resolve for everyone, so this is never deferred to the programmer.
