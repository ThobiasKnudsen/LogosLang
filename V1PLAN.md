...
*Seed status + sub-order (2026-07):* the operator core is done: `a = a + 1` parses, runs, and JIT-matches the interpreter (`=`, `+`, `rational`, `i32`, `fn` as identities; parse constructors are `Atom`/`Prefix`/`Infix` in the parser's `metas` table; the parser is reentrant with a local tape). The `fn` surface is pinned in DESIGN (›A function's surface is `fn (params) -> type (body)`‹ and ›`return` is explicit and yields to the nearest consuming scope‹). Real `fn` builds in this sub-order, each on the last: **(1) brackets/scopes + `return`** — `( )` opens/closes a scope (push/pop the scope stack) and delimits a sub-parse; `return` yields the scope's value, unwinding to the nearest *consumer* scope (`:=` RHS, fn body, operand) past *transparent* ones (loop/discarded); **(2) `struct`** — a `( name : type, ..., or bare name )` field list, which *is* the parameter list; **(3) `fn`** — `fn (params) -> ret (body)` composing (1) and (2), replacing the current placeholder prefix `fn <grabs-rest>`. `return` is required, so the simplest function is `fn () -> i32 ( return 40 + 2 )`.
