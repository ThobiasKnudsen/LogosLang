...
mod tests {
    ...
    fn parses_a_bare_name_field() {
        ...
        let node = {
            let mut p =
                Parser::new("logos (value = (t))", &mut store, &mut trie, core.types(), scopes);
            p.parse_expression().unwrap()
        };
    ...
    fn logical_operators_short_circuit_on_the_interpreter() {
        ...
        let and_sc = {
            let mut s = ScopeStack::new();
            s.push(core.root_scope);
            let mut p = Parser::new("false and y < 1", &mut store, &mut trie, core.types(), s);
            p.parse_expression().unwrap()
        };
        ...
        let or_sc = {
            let mut s = ScopeStack::new();
            s.push(core.root_scope);
            let mut p = Parser::new("true or y < 1", &mut store, &mut trie, core.types(), s);
            p.parse_expression().unwrap()
        };
    ...
    fn if_over_a_parameter_matches_between_tiers() {
        ...
        for (arg, expect) in [(0i64, 100i64), (5, 200)] {
            ...
            // SAFETY: `func` is the fn node just built and outlives the call.
            let _compiled =
                unsafe { compile_fn(&mut store, &core.lower, core.types(), func) }.unwrap();
            let jit = unsafe { rt.run(call) }.unwrap();
    ...
    fn compile_is_reserved_only_on_fn_typed_values() {
        ...
        assert_eq!(
            run_script("point := logos (value = (compile := i32 ?)),\np := point(7),\np.compile"),
            7
        );
    }
    ...
    /// Declare `point := logos (value = (x := i32 ?, y := i32 ?))` in the root scope.
    fn declare_point(store: &mut Store, trie: &mut RegexTrie, core: &Core) {
        let mut s = ScopeStack::new();
        s.push(core.root_scope);
        let mut p = Parser::new(
            "point := logos (value = (x := i32 ?, y := i32 ?))",
            store,
            trie,
            core.types(),
            s,
    ...
    fn record_fields_lay_out_mixed_widths() {
        ...
        let core = Core::build(&mut store, &mut trie);
        {
            ...
            s.push(core.root_scope);
            let mut p = Parser::new(
                "cell := logos (value = (a := u8 ?, b := i64 ?, c := i32 ?))",
                &mut store,
                &mut trie,
                core.types(),
                s,
    ...
    fn compiled_function_calls_another_compiled_function() {
        ...
        let _compiled_outer =
            unsafe { compile_fn(&mut store, &core.lower, core.types(), outer) }.unwrap();
        let jit = unsafe { rt.run(call) }.unwrap();
