...
//! `scope`, `start`, `end`, `gate`, `name`, and `lex_rank` (14 September
//! 2026, #120 and #122), so `a:scope` is where `a` was declared, `a:name`
//! its spelling, `a:lex_rank` the spelling's rank (a place `=` may write),
//! and `a:dyad` its cell, the dyad view, whose own two fields keep their
//! names: `a:dyad.type`, `a:dyad.value`. `:` is the field read on the
