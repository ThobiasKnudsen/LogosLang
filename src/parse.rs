...
impl<'a> Parser<'a> {
    ...
    fn shared_member(&mut self, at: usize) -> Result<(), ParseError> {
        ...
        if !self.is_slot_fill(item) {
            ...
            let name = unsafe { Record::spelling(crate::identities::declare::binding_of(item)) };
            if self
                .scopes
    ...
    unsafe fn path_operand(&mut self, node: DyadPtr) -> DyadPtr {
        use crate::identities::read::{read_kind, Read};
        let types = self.types;
        let runs = (*node).ty != types.record_
        ...
        while let Some(item) = self.parse_next() {
            ...
            unsafe {
                ...
                if (*node).ty == self.types.declare_ {
                    let name = Record::spelling(crate::identities::declare::binding_of(node));
                    let resolved = self
                        .scopes
                        .resolve(self.trie, &name)
    ...
    /// On a record, `dyad` is the view `{type: dyad, value: <the cell>}`,
    /// `start` the declaring line once it is complete, and any other field the
    /// instance-field read in `record`'s own scope; on a constructed node the
    ...
    unsafe fn record_read(
        ...
        let lhs = if (*lhs).ty == types.record_ {
            lhs
        } else {
        ...
        if (*lhs).ty == types.record_ {
            ...
            let start = Record::read(lhs).start;
    ...
    ///
    /// # Safety
    /// `value` must be a dyad from the store; `root` null or a record dyad.
    unsafe fn known_node(&self, value: DyadPtr, root: DyadPtr) -> Option<DyadPtr> {
        use crate::identities::{declare, read};
        let types = self.types;
        if !root.is_null() {
            if Record::has_gate(root, types.mut_) {
                return None;
            }
            let start = Record::read(root).start;
            if start.is_null()
            ...
            let next = if (*rhs).ty == types.record_ { rhs } else { std::ptr::null_mut() };
