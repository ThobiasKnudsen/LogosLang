//! `>=`: the *abstract* greater-than-or-equal comparison, an abstraction over all
//! machine comparisons (mirrors [`crate::identities::lt`]). At parse time it resolves
//! from its operands' types to a concrete op (`ge_i32` today), which it stores as its
...
unsafe fn concrete_op(node: DyadPtr) -> DyadPtr {
    *((*node).value as *const DyadPtr).add(GE_CONCRETE)
}
