//! `>`: the *abstract* greater-than comparison, an abstraction over all machine
//! comparisons (mirrors [`crate::identities::lt`]). At parse time it resolves from
//! its operands' types to a concrete op (`gt_i32` today), which it stores as its
...
//! the other relational operators (looser than arithmetic, tighter than `=`).
...
unsafe fn concrete_op(node: DyadPtr) -> DyadPtr {
    *((*node).value as *const DyadPtr).add(GT_CONCRETE)
}
