//! `==`: the *abstract* equality comparison, an abstraction over all machine
//! comparisons (mirrors [`crate::identities::lt`]). At parse time it resolves from
//! its operands' types to a concrete op (`eq_i32` today), which it stores as its
//! third operand, so `==` stays reflectable while run and compile delegate to that
//! concrete op ([`crate::identities::cmp`]). Its result is a `bool`; equality binds
//! looser than the relational operators (so `a < b == c < d` groups the comparisons
//! first) but tighter than `=`. The trie longest-matches `==` over `=`.
...
/// The index, in an `==` node's value struct, of the resolved concrete op.
const EQ_CONCRETE: usize = 2;
...
fn build(
    ...
    let concrete = types.eq_i32;
    let value = store.alloc_operands(&[lhs, rhs, concrete]);
    Ok(store.alloc_raw(eq, value))
}

/// The concrete op an `==` node resolved to (its third operand).
///
/// # Safety
/// `node` must be an `==` node built by [`build`], with a `[lhs, rhs, concrete]` value.
unsafe fn concrete_op(node: DyadPtr) -> DyadPtr {
    *((*node).value as *const DyadPtr).add(EQ_CONCRETE)
}
