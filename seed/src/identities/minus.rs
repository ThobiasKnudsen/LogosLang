//! `-`: the *abstract* subtraction operator, an abstraction over all machine
//! subtractions (mirrors [`crate::identities::plus`]). At parse time it resolves
//! from its operands' types to a concrete op (`sub_i32` today), which it stores as
//! its third operand, so `-` stays reflectable while run and compile delegate to
//! that concrete op ([`crate::identities::sub`]).
...
const MINUS_CONCRETE: usize = 2;

/// Register `-`: spelling and parse precedence (same as `+`, left-associative),
/// plus its resolve-and-delegate run and lowering. `-` carries no arithmetic of
/// its own; `sub_i32` (and future concrete ops) do.
...
/// Build `lhs - rhs`: resolve the concrete subtraction op from the operand types
/// and store it as the node's third operand, giving `{ty: -, value: [lhs, rhs, op]}`.
fn build(
    store: &mut Store,
    types: &CoreTypes,
    minus: DyadPtr,
...
unsafe fn concrete_op(node: DyadPtr) -> DyadPtr {
    *((*node).value as *const DyadPtr).add(MINUS_CONCRETE)
}
