//! `*`: the *abstract* multiplication operator, an abstraction over all machine
//! multiplications (mirrors [`crate::identities::plus`]). At parse time it resolves
//! from its operands' types to a concrete op (`mul_i32` today), which it stores as
//! its third operand, so `*` stays reflectable while run and compile delegate to
//! that concrete op ([`crate::identities::mul`]). `*` binds tighter than `+`/`-`.
...
const TIMES_CONCRETE: usize = 2;

/// Register `*`: spelling and parse precedence (binding tighter than `+`/`-`,
...
/// Build `lhs * rhs`: resolve the concrete multiplication op from the operand types
...
unsafe fn concrete_op(node: DyadPtr) -> DyadPtr {
    *((*node).value as *const DyadPtr).add(TIMES_CONCRETE)
}
