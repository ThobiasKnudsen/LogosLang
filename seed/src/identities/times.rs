...
//! `*`: multiplication. Like `+` (see [`crate::identities::plus`]), it stores its
//! resolved operand type in the value slot `{ty: *, value: [lhs, rhs, type]}` and
//! run/compile switch on it. Binds tighter than `+`/`-`, left-associative.
...
/// Register `*`: spelling, precedence (binding tighter than `+`/`-`, left-associative),
...
/// Run: multiply in the stored operand type.
fn run(rt: &mut Runtime, node: DyadPtr) -> Result<i64, RunError> {
    // SAFETY: `node` is a valid `*` application `[lhs, rhs, type]`.
    unsafe { eval_arith(rt, node, ArithOp::Mul) }
}

/// Lower: emit the machine multiplication for the stored operand type.
